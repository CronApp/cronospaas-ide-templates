                                    ...
                                    <#elseif (!gField.isPrimaryKey() || field.getClazz().hasCompositeKey())>
                                        ...
                                        <div data-component="${dataComponentType}"  id="crn-textinput-${gField.getDbFieldName()}-${model.random}" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                            <div class="form-group">
                                                ...
                                                <#if gField.isBoolean() >
                                                    <input type="checkbox"<#if gField.isNullable()>crn-allow-null-values="true"<#else>crn-allow-null-values="false"</#if> ng-model="${field.getName()}Grid.active.${gField.getName()}"  id="textinput-modal-${gField.getDbFieldName()}" placeholder="<#if gField.label?has_content>${gField.label}<#else>${gField.name?capitalize}</#if>" <#if !gField.isNullable()>required="required"</#if>>
